...
export default function TutorFloatingMenu() {
  ...
  // ✨ ซ่อนปุ่มนี้ถ้าอยู่หน้า Dashboard หลักของติวเตอร์
  if (pathname === '/tutor') {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-24 right-5 lg:bottom-10 lg:right-10 z-[999] flex flex-col items-end gap-3">
      ...
      <button 
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-[0_10px_30px_rgba(37,99,235,0.4)] transition-all duration-300 relative z-10 hover:scale-105 active:scale-95 border-2 border-white ${isOpen ? 'bg-slate-800 text-white shadow-slate-500/40' : 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white'}`}
      >
