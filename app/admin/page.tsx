...
import { 
  ...
  ArrowRight,
  Book,
  Landmark // ✨ เพิ่มไอคอน Landmark สำหรับการถอนเงิน
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({ tutors: 0, hours: 0, students: 0, pendingRedeems: 0, pendingWithdraws: 0 }); // ✨ เพิ่ม pendingWithdraws
  ...
  useEffect(() => {
    const fetchAdminAndStats = async () => {
      ...
      // ✨ ดึงจำนวนคำขอถอนเงินที่รออนุมัติ
      ...
      setStats({
        ...
        pendingWithdraws: withdrawCount || 0 // ✨ เก็บค่าคำขอถอนเงิน
      });
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col lg:flex-row font-sans text-gray-900">
      ...
      `}>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto no-scrollbar">
          ...
          <Link href="/admin" className="flex items-center gap-3 px-4 py-3.5 bg-blue-50 text-blue-600 rounded-2xl font-bold transition-all"><LayoutDashboard size={20} /> แผงควบคุม</Link>
          
          <Link href="/admin/users" className="flex items-center gap-3 px-4 py-3.5 text-gray-500 hover:bg-indigo-50 hover:text-indigo-600 rounded-2xl font-bold transition-all"><UserCheck size={20} /> จัดการผู้ใช้งาน</Link>

          <Link href="/admin/my-books" className="flex items-center gap-3 px-4 py-3.5 text-orange-600 hover:bg-orange-50 rounded-2xl font-bold transition-all"><Book size={20} /> คลังหนังสือรายคน</Link>

          <Link href="/admin/wallets" className="flex items-center gap-3 px-4 py-3.5 text-gray-500 hover:bg-blue-50 hover:text-blue-600 rounded-2xl font-bold transition-all"><Wallet size={20} /> จัดการกระเป๋าเงิน (6-Tier)</Link>
          
          {/* ✨ เมนูใหม่: เชื่อมหน้าถอนเงิน */}
          <Link href="/admin/withdrawals" className="flex items-center gap-3 px-4 py-3.5 text-emerald-600 hover:bg-emerald-50 rounded-2xl font-bold transition-all relative">
            <Landmark size={20} /> ตรวจสอบการถอนเงิน
            {stats.pendingWithdraws > 0 && (
              <span className="absolute right-4 bg-red-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-full animate-pulse">{stats.pendingWithdraws}</span>
            )}
          </Link>

          <Link href="/admin/orders" className="flex items-center gap-3 px-4 py-3.5 text-gray-500 hover:bg-green-50 hover:text-green-600 rounded-2xl font-bold transition-all"><Receipt size={20} /> ตรวจสอบการแจ้งโอน</Link>

          <Link href="/admin/redeems" className="flex items-center gap-3 px-4 py-3.5 text-gray-500 hover:bg-orange-50 hover:text-orange-600 rounded-2xl font-bold transition-all relative">
            <Gift size={20} /> จัดการการแลกรางวัล
          </Link>

          <div className="h-px bg-gray-50 mx-4 my-4"></div>

          <Link href="/admin/manage-tutors" className="flex items-center gap-3 px-4 py-3.5 text-gray-400 hover:text-gray-600 font-bold transition-all"><Users size={20} /> จัดการติวเตอร์</Link>
          <Link href="/admin/manage-courses" className="flex items-center gap-3 px-4 py-3.5 text-gray-400 hover:text-gray-600 font-bold transition-all"><BookOpen size={20} /> จัดการคอร์สเรียน</Link>
        </nav>
        ...
        <div className="p-4 md:p-8 lg:p-12 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 mb-8 md:mb-12">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-md transition-all relative overflow-hidden group">
              ...
              <p className="text-gray-400 font-black text-[10px] uppercase tracking-[0.2em]">รายการสั่งซื้อคอร์ส</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[3rem] p-8 md:p-10 text-white shadow-xl shadow-blue-200 relative overflow-hidden flex flex-col justify-between group h-[340px]">
               <div className="relative z-10">
                  <h2 className="text-3xl md:text-4xl font-black mb-4 leading-tight text-white">จัดการกระเป๋าเงิน<br/>6 Tier</h2>
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                {/* ✨ เมนูทางลัดใหม่: ตรวจสอบการถอนเงิน (สีเขียวเหนี่ยวทรัพย์) */}
                <Link href="/admin/withdrawals" className="bg-gradient-to-r from-emerald-500 to-teal-600 p-7 rounded-[2.5rem] flex items-center justify-between hover:shadow-xl hover:-translate-y-1 transition-all group relative overflow-hidden">
                  <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                      <h4 className="font-black text-white text-xl leading-tight">ตรวจสอบการถอนเงิน</h4>
                      <p className="text-[10px] font-bold text-emerald-100 uppercase mt-1 tracking-widest">
                        อนุมัติยอดขาย • {stats.pendingWithdraws} รายการรอดำเนินการ
                      </p>
                    </div>
                  </div>
                ...
                {/* จัดการคลังหนังสือรายคน */}
                <Link href="/admin/my-books" className="bg-white p-7 rounded-[2rem] border-2 border-orange-50 flex items-center justify-between hover:bg-orange-50 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h4 className="font-black text-gray-900 text-xl leading-tight">จัดการคลังหนังสือรายคน</h4>
                      <p className="text-[10px] font-bold text-gray-400 uppercase mt-1 tracking-widest">
                        เพิ่มชีทเรียน • แจกไฟล์เอกสาร
                      </p>
                    </div>
                  </div>
                ...
                <Link href="/admin/redeems" className="bg-white p-7 rounded-[2rem] border-2 border-purple-50 flex items-center justify-between hover:bg-purple-50 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 bg-purple-100 rounded-2xl flex items-center justify-center text-purple-600 group-hover:scale-110 transition-transform">
                      <Gift size={28} />
                    </div>
                    <div>
                      <h4 className="font-black text-gray-900 text-xl leading-tight">จัดการการแลกรางวัล</h4>
                      <p className="text-[10px] font-bold text-gray-400 uppercase mt-1 tracking-widest">
                        แต้ม Affiliate • {stats.pendingRedeems} รายการใหม่
                      </p>
                    </div>
                  </div>
                  <ChevronRight className="text-purple-300 group-hover:text-purple-600 group-hover:translate-x-1 transition-all" />
                </Link>
